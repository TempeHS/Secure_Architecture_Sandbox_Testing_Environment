...
python src/analyzer/dast_cli.py http://localhost:5000 --tools custom
...
# Timestamped report
python src/analyzer/dast_cli.py http://localhost:5000 > reports/dast_$(date +%Y%m%d_%H%M).txt
```
...
python src/analyzer/dast_cli.py http://localhost:8000
```
...
**Runtime Testing**: Analyze applications while they're running
- Tests from attacker's perspective (black-box testing)
- Sends malicious payloads to identify vulnerabilities
- Tests actual application behavior and responses
...
- Checks server configurations and files
...
- Consider application context and intended behavior
...
**Response Content**: Analyze application responses
