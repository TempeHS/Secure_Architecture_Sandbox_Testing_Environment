...
python src/analyzer/network_cli.py --scan-services localhost > reports/network_$(date +%Y%m%d_%H%M).txt
...
- Analyze protocol usage and communication patterns
- Identify anomalous or suspicious network behavior
...
python src/analyzer/network_cli.py --demo-network  # Use simulated data
```
