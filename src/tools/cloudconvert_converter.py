...
class CloudConvertWorksheetConverter:
    ...
    def validate_api_key(self) -> bool:
        ...
        try:
            # Simple validation - check if API key exists and is in JWT format
            if not self.api_key:
    ...
    def convert_markdown_to_docx(self, input_file: Path, output_file: Path) -> bool:
        ...
        try:
            ...
            upload_result = Task.upload(str(input_file), upload_task)
    ...
    def get_account_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.api_key.startswith('eyJ'):
                return {
                    'status': 'API key is present and appears valid',
                    'api_key_format': 'JWT token',
                    'api_key_length': len(self.api_key),
                    'ready_for_conversion': True,
                    'note': 'Full account details require additional API scopes'
                }
            else:
                return {
                    'status': 'API key format may be invalid',
