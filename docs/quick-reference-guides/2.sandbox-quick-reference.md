...
# Check for unauthorised network activity
...
python src/analyser/analyse_cli.py target_file --output reports/sandbox_report.json --format json

# Save findings to file
python src/analyser/analyse_cli.py target_file > reports/sandbox_report_$(date +%Y%m%d).txt
```
...
- Focus on unexpected or unauthorised actions
...
```bash
# Basic endpoint testing (backdoor sample app listens on 6000)
curl http://localhost:6000/
curl http://localhost:6000/about

# Directory enumeration
dirb http://localhost:6000/ /usr/share/dirb/wordlists/common.txt

# Real hidden backdoor paths in this application
...
docker-compose -f docker/docker-compose.yml restart cybersec_sandbox
