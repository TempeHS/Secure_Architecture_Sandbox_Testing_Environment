...
# Legitimate-looking behaviour
for i in range(10):
    print(f"Optimising system component {i+1}/10...")
    time.sleep(1)
...
print("Performing routine system optimisation...")
...
- Like a "sealed laboratory" for studying malicious behaviour
...
- DAST: Tasting the food (testing the running programme)  
- Sandbox: Watching the cook's behaviour (monitoring what software actually does)
...
```
Student Computer → Codespaces → Docker → Sandbox Network
```

### 🔬 Hands-On Activities

**Activity 1: Basic Behavioural Analysis**
```bash
...
# Analyse the trace for suspicious behaviours
...
```bash
# Start the backdoor sample app (listens on port 6000)
python samples/backdoor-apps/backdoor_app.py &

# Probe the disguised endpoints
...
**Behavioural Indicators to Highlight:**
...
- Emphasise patterns over exact matches
...
curl "http://localhost:6000/admin_debug_console?cmd=ls%20-la"

# Test 3: Show system information (VERY DANGEROUS in real life)
curl "http://localhost:6000/admin_debug_console?cmd=cat%20/etc/passwd"
```
...
**🎯 Teaching Tip**: Emphasise this is only safe because we're in our sandbox.
...
**🎯 Teaching Tip**: Walk around and help students organise their thoughts. This
...
- **Multiple processor cores utilised** (maximising theft)
...
| **Sandbox** | Watches software behaviour  | When software might be dangerous |

**❌ Challenge**: Students miss important malicious behaviours **✅ Teaching
Strategy**:
...
1. **Multi-stage Malware**: Analyse malware that downloads additional payloads
