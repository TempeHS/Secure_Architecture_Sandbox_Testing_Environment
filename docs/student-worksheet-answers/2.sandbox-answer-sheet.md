...
- **What script claims to do**: System optimisation, performance improvement,
  cleanup
...
- **http://localhost:6000/**: Normal-looking company homepage
- **http://localhost:6000/about**: About page
- **http://localhost:6000/contact**: Contact page

**Hidden Endpoints Discovery**: **Command**:
`gobuster dir -u http://localhost:6000 -w /usr/share/wordlists/common.txt`

**Expected Hidden Endpoints**:

- `/admin_debug_console` - Command execution backdoor
- `/system_health_check` - Disguised command form
- `/backup_utility` - Arbitrary file read
...
```bash
# Command 1: Test the disguised admin console
curl "http://localhost:6000/admin_debug_console?cmd=whoami"
# Result: Arbitrary command execution without authentication

# Command 2: Test the disguised file reader
curl "http://localhost:6000/backup_utility?action=read&file=/etc/passwd"
# Result: Arbitrary file read capability revealed
```
...
| 10             | **80-95%**    | **Increasing** | **High CPU utilisation**    |
...
**4. Organisational Defence Strategy**:
...
- **Expected Answer**: Legal liability for unauthorised analysis, need for
  proper authorisation and scope, compliance with computer crime laws,
...
- **Expected Answer**: Maintain confidentiality, avoid unauthorised access,
...
importance of proper authorisation. Ensure students understand the legal and
