...
python src/analyser/analyse_cli.py samples/unsecure-pwa --tools all --quick --output quick_sast_scan.pdf --format pdf --verbose
...
python src/analyser/analyse_cli.py target_app --format summary

# Export findings for integration
python src/analyser/analyse_cli.py target_app --format csv > findings.csv
## Concepts
...
**Access Control Problems**: Missing authorization cheques
...
python src/analyser/analyse_cli.py target --confidence-level high
```
