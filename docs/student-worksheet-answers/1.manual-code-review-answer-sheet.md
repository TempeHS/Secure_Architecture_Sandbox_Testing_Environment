...
- **Location:** `retrieveUsers` function, line 20
...
- **Location:** `retrieveUsers` function, line 25
...
2. **What happens if an attacker uses different usernames for each check?**
   **Answer:** **Authentication bypass is possible. Attacker can:**
   ...
   - **Mix valid user credentials to gain unauthorised access**
...
2. **Can users access functionality they shouldn't?** **Answer:** **YES - No
   proper session management or authorisation checks on routes**
...
| ------------------------ | -------------------------------- | ------------ | -------------------------------- | --------------------------- |
| **SQL Injection**        | user_management.py:20,25         | **Critical** | String formatting in SQL queries | Use parameterised queries   |
| **Stored XSS**           | user_management.py:listFeedback  | **High**     | Unescaped HTML output            | HTML encode user content    |
| **Reflected XSS**        | templates/index.html             | **High**     | Safe filter disables escaping    | Remove the safe filter      |
