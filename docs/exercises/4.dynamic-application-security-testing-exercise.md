...
```
docker/  docs/  reports/  samples/  src/  tests/  README.md  WELCOME.md  ...
```
...
```
usage: dast_cli.py [-h] [--demo-apps] [--tools {nikto,gobuster,basic_tests,all} ...]
                   [--deep-scan] [--quick] [--output OUTPUT]
                   [--format {json,txt,md,pdf}] [--educational] [--verbose]
                   [--quiet] [target_url]
...
options:
  ...
  --demo-apps           Scan all demo applications
  --tools               Specific tools to run (nikto, gobuster, basic_tests, all)
...
```bash
python src/analyser/dast_cli.py http://localhost:5000 --quick --educational --output quick_dast_unsecure_pwa.pdf --format pdf --verbose
```
...
```bash
python src/analyser/dast_cli.py http://localhost:9090 --educational --format json --output flask_dast_report.json
```
