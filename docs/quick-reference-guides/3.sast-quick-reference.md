...
```bash
# Analyse application with educational output (RECOMMENDED)
...
python src/analyser/analyse_cli.py samples/unsecure-pwa --tools all --output quick_sast_scan.pdf --format pdf --verbose

# Generate detailed JSON report
python src/analyser/analyse_cli.py samples/unsecure-pwa --tools all --output report.json --format json --verbose

# Comprehensive analysis with multiple output formats
python src/analyser/analyse_cli.py samples/unsecure-pwa --tools all --educational --verbose --output comprehensive_sast_report.pdf --format pdf
```
...
python src/analyser/analyse_cli.py target_app > reports/sast_$(date +%Y%m%d_%H%M).txt

# Generate executive summary
python src/analyser/analyse_cli.py target_app --format md --output reports/sast_summary.md

# Export findings for integration
python src/analyser/analyse_cli.py target_app --format json --output reports/findings.json
```
...
- Analyses code patterns and structures
- Finds issues before deployment

**Key SAST Principles**:
- **Early Detection**: Find vulnerabilities during development
- **Comprehensive Coverage**: Analyse entire codebase systematically
...
**Access Control Problems**: Missing authorisation cheques
...
- Cheques for dangerous function usage
...
```bash
# Fix: Review findings manually and re-run a single trusted tool
python src/analyser/analyse_cli.py target --tools bandit --educational
```
...
- Use `--tools bandit` to narrow the scan to a single tool
...
- Prioritise based on confidence levels
...
1. Prioritise fixes based on risk
...
python -c "import src.analyser.analyse_cli"
```
...
chmod +x src/analyser/analyse_cli.py
...
4. **Fix systematically** - prioritise by risk, not by count
...
- Can interpret and prioritise security findings
