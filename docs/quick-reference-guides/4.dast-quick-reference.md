...
# Built-in DAST probes (XSS, SQL injection, security headers)
python src/analyser/dast_cli.py http://localhost:5000 --tools basic_tests
...
- **"Target unreachable - no security testing was performed"**: The scan reached
  nothing and exits with code 2. This is NOT a clean result - start the
  services and check the port with `curl -I <url>`
