...
python src/analyser/dast_cli.py http://localhost:9090 --educational > flask_security_scan.txt
python src/analyser/dast_cli.py http://localhost:5000 --educational > pwa_security_scan.txt
```
...
- Did the scan print "Target unreachable - no security testing was performed"?
  That is not a clean result - the scan reached nothing and exits with code 2.
  Start the services and re-run.
