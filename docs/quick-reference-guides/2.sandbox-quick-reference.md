...
```bash
# Analyze suspicious script safely
python src/analyzer/sandbox_cli.py samples/suspicious-scripts/fake_game.py
...
python src/analyzer/sandbox_cli.py samples/unsecure-pwa/
...
python src/analyzer/sandbox_cli.py target_file --report

# Save findings to file
python src/analyzer/sandbox_cli.py target_file > reports/sandbox_report_$(date +%Y%m%d).txt
## Concepts

### Sandbox Security Principles

**Application Isolation**: Containers provide secure boundaries for analyzing untrusted code
- Applications run in controlled environment
- Cannot access host system resources
- Network activity is monitored and contained

**Behavioral Analysis**: Monitor application actions to detect threats
...
**Threat Detection**: Identify malicious behavior through observation
...
**Static Behavioral Prediction**: Examine code for potential threats before execution
**Dynamic Behavioral Monitoring**: Observe actual behavior during controlled execution
...
**False Positives**: Normal application behavior vs. threats
