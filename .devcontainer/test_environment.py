...
def test_command_tool(tool_name, command, expected_in_output=None):
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            print_error(f"{tool_name}: Command failed (return code: {result.returncode})")
            return False
    ...
    for var_name, expected in env_vars:
        ...
        if actual:
            if expected and expected not in actual:
                print_warning(f"{var_name}: Set to '{actual}' (expected to contain '{expected}')")
            else:
...
def run_comprehensive_test():
    ...
    print_info("This script will verify that all tools and dependencies are properly installed")
