...
**Expected Command**: In a second terminal, run `python samples/network-scenarios/basic_network_activity.py 60` and press Enter when prompted.
...
- **Geographic Distribution**: Expected organisational/ISP ranges
...
**Expected Command**: In a second terminal, run `python samples/network-scenarios/suspicious_traffic_generator.py 60` and press Enter when prompted.
**Expected Command**: `python src/analyser/network_cli.py --capture-traffic --duration 60`
...
**Expected Command**: In a second terminal, run `python samples/network-scenarios/backdoor_simulation.py 60` and press Enter when prompted.
...
**Expected Command**: In a second terminal, run `python samples/network-scenarios/dns_threat_scenarios.py 60` and press Enter when prompted.
**Expected Command**: `python src/analyser/network_cli.py --dns-analysis --duration 60`

**DNS Security Threats Detected**:
- **DNS Tunnelling** - Excessive DNS queries with large payloads
...
- **Time Synchronisation** - Accurate timestamps for correlation
- **Data Integrity** - Cryptographic hashes to prevent tampering
- **Collection Authorisation** - Proper legal authority for network monitoring
...
## 📋 Investigate Security Scenario (Worksheet Exercise 5) - Answer Key

### Investigation Phase - Expected Commands

1. **Port 4444 check**: `netstat -tuln | grep 4444`
2. **DNS query analysis**:
   `python src/analyser/network_cli.py --dns-analysis --duration 120 --educational`
3. **Comprehensive report** (network_cli.py runs one mode per invocation):
...
### Investigation Findings - Expected Results

Students should connect three independent pieces of evidence rather than
relying on any single one:

- **Port 4444** is the Metasploit/Meterpreter default listener port. Outbound
  connections to it from multiple internal hosts to the *same* external IP is
  a classic command-and-control (C2) beaconing indicator.
- **DNS analysis** should surface the suspicious domain patterns generated by
  `samples/network-scenarios/dns_threat_scenarios.py` — DGA-style random subdomains,
  unusually long labels (possible DNS tunnelling), and low-reputation TLDs.
- **The comprehensive report** ties the two together with timestamps, showing
  DNS resolution of the suspicious domain immediately preceding the port 4444
  connection.

**Assessment question — genuine compromise or innocent explanation?**

Strong answers argue **likely compromise** and justify it with the
*correlation*: a single host on port 4444 could be a developer running a test
listener, but *multiple* hosts beaconing to one external IP, on a known C2
port, correlated in time with DGA-like DNS queries, is very difficult to
explain innocently. Excellent answers also state what would *disprove* it —
e.g. a documented change request, or the IP resolving to a known internal
testing service.

**Immediate containment recommendation**

Expected answers include: block the external IP at the perimeter firewall,
sinkhole/blocklist the suspicious domains at the DNS resolver, isolate the
affected hosts from the network (but do **not** power them off — this destroys
volatile memory evidence), preserve logs and packet captures, then escalate to
incident response for forensic imaging.

---

## 📋 TLS/DNS Stream Visualisation (Worksheet Exercise 6) - Answer Key

### DNS Lookup - Expected Results

**Expected Command**: `dig example.com`
...
**Teaching Note**: `127.0.0.53` is a local DNS stub resolver (common on
Debian/Ubuntu-based systems) — it caches answers and forwards uncached
queries upstream. The 5ms "Query time" here reflects that local hop, not the
full round-trip to an authoritative DNS server, which is why this can look
much faster than the `namelookup` figure students see for the same domain in
their very first `curl -w` run (before anything is cached) versus later runs
(where the OS/resolver cache makes it near-instant).

### Phase Timing - Expected Results
...
**Teaching Note**: Absolute values vary by network path and site, but the
values should always be monotonically increasing (`namelookup` ≤ `connect` ≤
`appconnect` ≤ `starttransfer` ≤ `total`), since each phase includes the time
of the phases before it.

### Full Stream Capture - Expected Results

**Expected Command**: `curl -v --http1.1 https://example.com`

**Teaching Note**: `curl -v` needs no extra tools or container permissions,
so it's the recommended command for showing the full TLS handshake and
headers.

**Sample output** (real, from `curl -v --http1.1 https://example.com`,
abridged):
```
...
Usually `appconnect` (TLS handshake) or `connect` (TCP) accounts for the
largest jump, especially on the first request to a new host (no cached DNS,
no existing TCP/TLS session). Sites behind a CDN (e.g. Cloudflare, Fastly)
often show faster `connect`/`appconnect` times because the client connects to
a geographically nearby edge server rather than the origin server.

**2. What information in the HTTP stream would be visible to a
man-in-the-middle if TLS were absent?**

Without TLS, everything after the TCP handshake — the full HTTP request line,
all headers (including cookies/session tokens/auth headers), and the entire
response body — would be visible in plaintext to anyone able to observe the
network path. This includes credentials submitted in forms, session
identifiers, and any sensitive data in the response.

**3. Why can't Codespaces show raw packet-level detail (e.g., TCP
retransmits, cipher suite negotiation) the way local Wireshark can?**

Codespaces runs inside a remote container without root-level access to the
underlying host's network interface, so tools that need raw socket/packet
capture privileges (like `tcpdump`/Wireshark in promiscuous mode) aren't
available. `curl -w` and `curl -v` both work at the application layer (they
see the request/response as the application sees it), not the packet layer,
so they don't need those elevated privileges — but they also can't show
things like retransmitted TCP segments or the raw TLS handshake bytes.
...
Large sites are usually served from many servers for load balancing and
redundancy — a domain's DNS record can list multiple IPs, and the client
(or an intermediate load balancer) picks one to connect to. Running `dig`
again can show a different order (many DNS servers rotate/randomise the
order of returned records, known as "round-robin DNS") or even different
addresses entirely if the site uses a CDN like Cloudflare, which returns
different edge server IPs based on where in the world the query came from.
...
**Expected Answer**: DNS is often the first indicator of malware communication, difficult to block completely without disrupting business, reveals command & control infrastructure, detects data exfiltration through DNS tunnelling.
...
**Expected Answer**: Network monitoring can detect IP theft and data exfiltration, unauthorised access to proprietary systems, industrial espionage and trade secret theft, protection of confidential business communications.
...
**Expected Answer**: Identify VPN traffic patterns and protocols, detect unauthorised tunnelling protocols, monitor for DNS tunnelling and data exfiltration, analyse encrypted channel establishment patterns.
...
**Expected Answer**:
- **Device Discovery**: Identifying and cataloguing IoT devices
