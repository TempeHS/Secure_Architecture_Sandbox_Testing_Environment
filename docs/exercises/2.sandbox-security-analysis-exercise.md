...
> [!IMPORTANT]  
> If you run `docker exec -it cybersec_sandbox bash` a second time, you will be running the command from inside the docker which will throw an error.
...
- **Evidence**: Command execution endpoints, unauthorised system access
- **Impact**: Remote control, privilege escalation, persistent access
- **Syllabus Connection**: Authentication bypass and authorisation flaws
