...
Static Application Security Testing (SAST) analyzes source code **without executing the program** to identify security vulnerabilities. This exercise teaches you to **apply security features incorporated into software** by using automated tools to detect common security issues and learn remediation techniques.
...
python3 src/analyzer/analyze_cli.py --help
```
...
```bash
# Analyze the unsecure PWA (primary target)
python src/analyzer/analyze_cli.py samples/unsecure-pwa
...
**Step 2.1: Analyze Input Validation Issues**
```bash
...
```bash
**Step 5.3: Prioritize and Document Findings**
```bash
# Generate summary report
python src/analyzer/analyze_cli.py samples/unsecure-pwa --educational > vulnerability_report.txt
...
- **Vulnerability interpretation**: Understanding and prioritizing security findings
...
```bash
python src/analyzer/analyze_cli.py samples/unsecure-pwa --output json
```
...
## 📋 Exercise 2: Analyzing the Unsecure PWA
...
```bash
python src/analyzer/analyze_cli.py samples/unsecure-pwa --educational | grep -A 5 -B 5 "SQL\|sql"
```
...
```bash
python src/analyzer/analyze_cli.py samples/unsecure-pwa --educational | grep -A 3 -B 3 "auth\|password\|login"
```
...
```bash
python src/analyzer/analyze_cli.py samples/unsecure-pwa --educational | grep -A 3 -B 3 "debug\|config\|secret"
```
